...
- CMake: `find_package(Zypp)` if a CMake config / pkg-config file ships, else
  `pkg_check_modules(ZYPP libzypp)` or a small Find module under `cmake/`. Record the
  mechanism and detected version in `TRANSLATION_REPORT.md` and verify on the host.
...
- CMake: `find_package(jsoncpp)` or `pkg_check_modules(JSONCPP jsoncpp)`.
...
- Use yaml-cpp (`libyaml-cpp`) for the opt-in YAML serialisation; present on both
  ...
  through 0.8, avoid 0.7+-only entry points. `find_package(yaml-cpp)` and branch on
  the discovered version in CMake if needed; record the version.
...
- 2026-06-01: Compressed losslessly from the accreted v0.5.0-v0.6.5 file (same rule
  ...
  yaml-cpp with the soname and 0.6-vs-0.8 caveats, libcrypto for SHA256, libsolv
  reserved for 0.7.0) are retained in full, as they are the load-bearing C++-specific
  content. Tracks spec v0.6.5: per-file baseline via `RpmHeader::tag_fileinfos()`/
  `FileInfo` (ghost bit included, no librpm, no exec), reproducibility emission rule
  (emit content-bearing ghosts and type-mismatched paths, suppress
  empty-ghost-matching-empty).
