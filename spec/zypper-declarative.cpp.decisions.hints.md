...
- 2026-06-02: Build fix (CMake glue, no spec/logic change): switched dependency
  ...
  is Meson-generated and omits `include(CMakePackageConfigHelpers)`, so its trailing
  `check_required_components(jsoncpp)` fails with "Unknown CMake command" and breaks
  the build; the `.pc` files are present and stable on both SPs. pkg-config is the
  robust cross-SP discovery and avoids this per-SP CMake-config drift. (Built fine on
  SLE 15 with a minor change; this unblocks SLE 16.)
